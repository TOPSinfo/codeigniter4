...
{
    /**
        * Function to show user dashboard with pagination,serach and other filters
        * @access_url
        * @access_method
    ...
    public function index()
    {
       ...
       // Call makeLinks() to make pagination links.
        ...
        'user_data'=>$user_data[0],
         ...
         echo view('User/UserDashboard',$data);
    }
    /**
    ...
    */
    ...
    public function create_user(){
                ...
                if(empty($check_email)){
                   ...
                   /* else{
                    ...
                    if($res){
                      ...
                      //unable to save  
                        $session->setTempdata('error', 'Sorry! Unable to create user.try again.',3);
                        return redirect()->to(current_url());
                    }
                }else{
        ...
        echo view('User/CreateUser',$data);
    }
    /**
    ...
    */
    public function delete_user(){
        ...
        if($res){
            $session->setTempdata('success', 'User deleted Successfully.',3);
            return redirect()->to('/userDashboard');
        }else{
    ...
    */
    public function edit_user($id){
        ...
        if($id){
            $data['user_name'] = $session->get('name');
            $user_data = $userModel->where(array('id'=>$id,'is_deleted'=>0))->find();
            $data['edit_user_data'] = $user_data[0];
        }
        if($this->request->getMethod()=='post'){
            $uid=$_POST['uid'];
             $rules=[
            ...
            if($this->validate($rules)){
                ...
                $username=$this->request->getVar('username');
                   ...
                   /* else{
                    ...
                    if($res){
                        $session->setTempdata('success', 'User details updated Successfully.',3);
                        return redirect()->to('/userDashboard');
                       
                    }else{
                        //unable to save  
                        $session->setTempdata('error', 'Sorry! Unable to update user details.try again.',3);
                        return redirect()->to(current_url());
                    }
    ...
    */
    public function change_status_user(){
        ...
        if($userModel->update($user_id, $update_data)){
            $session->setTempdata('success', 'User status has been changed successfully.',3);
            return redirect()->to('/userDashboard');
    ...
    */
     public function change_bulk_status_user(){
